...
(test-group "contours"
  ...
  (let* ((img (load-image "cremate.jpg"))
    ...
    (test-assert (save-image! img (create-temporary-file ".jpg")))
    (test-assert (draw-point! img (seq-ref contours 0) 10 white 3))
    (test 354 (length (c->list contours)))
    (test 354 (length (seq.h_next->list contours)))
    (test 1 (seq.total contours))
    (test 1 (length (seq->list contours)))
    (let ((point (seq-ref contours 0)))
      (test 45 (cvpoint.x point))
      (test 308 (cvpoint.y point)))
    (test 0.0 (arc-length contours))
    (test 0.0 (contour-area contours))
    (test-assert (seq-ref hull 0))
